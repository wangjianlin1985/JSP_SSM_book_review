...
public class ProductClassController extends BaseController {
	...
	public void listAll(HttpServletResponse response) throws Exception {
		...
		for(ProductClass productClass:productClassList) {
			...
			jsonProductClass.accumulate("productName", productClass.getProductName());
			jsonArray.put(jsonProductClass);
		}
	...
	public String delete(@PathVariable Integer productClassId,HttpServletRequest request) throws UnsupportedEncodingException {
		  try {
			  productClassService.deleteProductClass(productClassId);
	            request.setAttribute("message", "图书分类删除成功!");
	            return "message";
	        } catch (Exception e) { 
	            e.printStackTrace();
	            request.setAttribute("error", "图书分类删除失败!");
				return "error";
	...
	public void OutToExcel( Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        ...
        String[] headers = { "图书分类id","图书分类名称"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<productClassList.size();i++) {
        	ProductClass productClass = productClassList.get(i); 
        	dataset.add(new String[]{productClass.getProductClassId() + "",productClass.getProductName()});
        }
        /*
